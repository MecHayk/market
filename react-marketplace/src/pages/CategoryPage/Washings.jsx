...
const Washings = () => {
  ...
  const status = useSelector((state) => state.washings.status);

  const { onAddToFavorite } = useContext(AppContext);

  const skeleton = [...new Array(10)].map((_, index) => <Skeleton key={index} />);
  const washingItems = washing.map((obj) => <Card onAddToFavorite={onAddToFavorite} {...obj} />);

  return (
    <div className={s.category}>
      <div className={s.title}>
        <h1>Стиральные машины</h1>
        <p>{washing.length} товаров</p>
      </div>

      <Sort />

      <div className={s.categoryWrapper}>{status === 'loading' ? skeleton : washingItems}</div>
